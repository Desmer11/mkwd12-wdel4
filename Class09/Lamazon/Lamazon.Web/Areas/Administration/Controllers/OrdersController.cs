...
namespace Lamazon.Web.Areas.Administration.Controllers
{
    public class OrdersController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<JsonResult> GetOrders(DatatableRequestViewModel datatableRequest)
        {
            var tableResult = await _orderService.GetFilteredOrders(datatableRequest);
        ...
        public async Task<IActionResult> RejectOrder(int? id)
        {
            if (id.HasValue)
            {
                await _orderService.RejectOrder(id.Value);
